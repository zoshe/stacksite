...
var setDiv = qDoc.createElement("DIV");
...
// needs to be changr to qSetMenu
...
setDiv.setAttribute("qAT"+arrayNum, menuType);
...
setArray[setArrayLength].unshift(X.replace(re, "href=\""+sitePath));
...
qssBuildTableFills(objX.qNum, objType, objSubType, 14, objX, 0, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,0); // make the borders for main frame object
...
qssBuildTableFills(objX.qNum, checkType, objSubType, 14, curNavLevObj, pullmenuIsneeded, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,selectIsNeeded); // make the buttons
...
var beforeSpaceText = qDoc.createTextNode(objIdArray[6]); // this extra space may need to go
...
var separatorText = qDoc.createTextNode(separatorChar);//don't no about the extra space
...
qssBuildTableFills(objX.qNum, objType, objSubType, 11, objX, 0, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,0); // make the borders for the object
...
//CSS
//alert(stackPath +"t/"+jsTagClassNames[11] + "/g.css");
qssAddCssLink( stackPath +"t/"+jsTagClassNames[11] + "/g.css?"+jsTagClassNames[4]);
qDocBody.insertBefore(qSizeDiv, qGBID("qBody"));
...
qDocBody.insertBefore(qDropMnuDiv, qGBID("qBody"));
qDocBody.insertBefore(qLayoutDiv.firstChild, qGBID("qBody"));
}
/*00NOTE
...
alert("it went to far. There should not be a qssBuildLayoutPart2");
}
...
qssBuildTableFills(objQumber+1,objType,objSubType,6, menu[c],true,"notUsed","notUsed",0,0,0,selectIsNeeded); // this fills in the menus for each box built
} // end of for loop
scrollBoxesNeeded = 0;
}
...
divX.style.background="url("+stackPath+"b/"+bgImageFolder+"/"+(trueBdrQty+1)+".gif) "+imgCssPostion[trueBdrQty]; }//isBorder
/*
...
var subtractHeightBy=(ii==0&&objType!=3)?qssGetPosElm(objTa)[1]:qssGetPosElm(chdTd)[1];//firstRow:bodyTable,middleRows
getChdTdHeight =  qssGetPosElm(chdNextTrTd)[1] - subtractHeightBy;
//(objType == 3)?alert("hello") : null; // still buggy here with FF
//alert("first||Middle||body table Row: "+ getChdTdHeight +"||||"+  qssGetPosElm(chdNextTrTd) +"||||"+ subtractHeightBy);
//alert(chdNextTrTd +"|||"+ chdNextTrTd.offsetTop);
}
else{// last table row
getChdTdHeight =  (qssGetPosElm(objTa)[1] + objTa.offsetHeight) - qssGetPosElm(chdTd)[1];
//alert("last table row: "+ getChdTdHeight +"||||"+(qssGetPosElm(objTa)[1] + objTa.offsetHeight) +"||||"+ qssGetPosElm(chdTd)[1]);
}
...
addHrSeperator();
addHrSeperator();
qBodyElm.appendChild(txNode("Body ||"+ qDocBody.innerHTML));
//addHrSeperator();
...
var h1 = qDoc.createElement('hr');     	//  MB_btnT top scroll button
...
qssBuildLayout(); // may need qWin.buildTime = setTimeout(buildTime(),qDoc.buildTimeSeconds);
//qssBuildLayoutPart1();
//qssBuildLayoutPart2();
