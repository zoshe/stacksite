...
var objTagName = objIdArray[0].indexOf(".")>-1 && objIdArray[0].split(".");
...
objX.tagNme = objTagName && objTagName[1]; // the name of the tag name to be used
...
var objParentChildQty = objParent.childQty; // get the current div's parent's Chlidren Qty //NOTE not used
var objParentType = objParent.objType; // get the parent type  //NOTE not used
var objParentSubType = objParent.objSubType; // get the parent sub type  //NOTE not used
var objParentTagNme = objParent.tagNme;
}//ift
...
qssBuildTableFills(objX.qNum, objType, objSubType, 4, objX, 0, objId, objTagName, objParentTagNme, objParent.qNum, objX.parentSubNum,0); // make the borders for main frame object
...
var objDiv =  qssGetElmInVar(qLayoutDiv,"q"+objX.qNum+"Ld",objX.tagNme);
//alert(objDiv);
if(!objX.getElementsByTagName("b")[0]){
...
qssBuildTableFills(objX.qNum, checkType, objSubType, 4, curNavLevObj, pullmenuIsneeded, objId,objTagName, objParentTagNme, objParent.qNum, objX.parentSubNum,selectIsNeeded); // make the buttons
...
// selectIsNeeded could be handled by adding chars to (objIdArray[X].indexOf("s"))?
var selectIsNeeded = true; // ***NEEDSWORK finds out if buttons have selected menu option on
...
else if(objType==90){ //NOT FINISHED // NEED WORK // NOT USED YET this builds a content object needs to go here
var placeItHere = qssGetElmInVar(qLayoutDiv,"q"+objParent.qNum + "Ld","DIV");
var contentNum = "qN"+objIdArray[5] || false;
var contentDiv = contentNum && qGBID("qN"+objIdArray[5]) || document.createElement("div");
contentDiv.setAttribute("zs",objId);
(contentNum)? null : contentDiv.innerHTML = "Content-Item-Here";
placeItHere.appendChild(contentDiv);
}
else{ // error
...
//CSS
//qssAddCssLink( stackPath +"t/"+qFileArray[13] + "/g.css");
...
if(qDocBody.firstChild){
qDocBody.insertBefore(Frag1,qDocBody.firstChild);
}
else{
qDocBody.appendChild(Frag1);
}
if(qBodyIsOn){
//alert(qDocBody.currentStyle && qDocBody.currentStyle.display);
//alert(window.getComputedStyle(qDocBody,null).display);
//qDocBody.style.display="table";
qDocElm.setAttribute('stacksite','loaded');
// addCssToDoc("html,body{border:0;padding:0;margin:0;height:100%;width:100%}body*{display:none}");
//alert("lll");
//addCssToDoc("body{display:block}");
}
//alert("qssBuildLayoutPart1-2");
// qqSnp qArray = qSetDiv = null; // To cut down on memory code. Not sure if this will have any benifit, Needs testing
qqSnp = null;
...
function qssBuildTableFills(objQumber,objType,objSubType, divqty, divObject, pointer_isneeded, objId, objTagName, objParentTagNme, parentqNum, parentSubNum, selectIsNeeded){
...
var divX = qDoc.createElement(objTagName[1] || "div");
//100028 var divX = qDoc.createElement(isObjTypeDivBox && "div" || "a");
...
//alert(cc);
var useTagName = objParentTagNme;
var inTheDiv = qLayoutDiv;
var placeithereId = "q"+parentqNum + "Ld";
var placeithere = qssGetElmInVar(inTheDiv,placeithereId,useTagName);
placeithere.appendChild(divX);
...
div.appendChild(txNode("HeadXX ||"+ qHead.innerHTML));
div.appendChild(addHrSeperator());
div.appendChild(txNode("BodyXX ||"+ qDocBody.innerHTML));
//addHrSeperator();
//div.appendChild(txNode("Menus ||"+ qGBID('qbodymen').innerHTML));
qDocBody.appendChild(div);

function txNode(Txt){return qDoc.createTextNode(Txt);}
function addHrSeperator(){return qDoc.createElement('hr');}
}
