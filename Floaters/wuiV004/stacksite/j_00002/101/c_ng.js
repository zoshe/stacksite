...
var qDocBodyClassName = "";// hold the body classname until it can be appended to the body tag
...
//100012 var checkTrQty = 3; // tr qty
//100012 var checkTdQty =1; // td qty
//0296 changed if(objX.hasChildNodes()){
if(objX.getElementsByTagName("b")[0]){
//NOTE ####### i think passing the alignClassName can go
...
qDocBodyClassName =  "q"+objX.qNum+"cz";
...
qssBuildTableFills(objX.qNum, objType, objSubType, 4, objX, 0, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,0); // make the borders for main frame object
//100012 qssBuildTable(objX.qNum, objType, objSubType, objParent.qNum , objX.parentSubNum,checkTrQty, checkTdQty,"notBeingUsed","notBeingUsed"); // make the table
...
qssBuildTable(objX.qNum, objType, objSubType, objParent.qNum , objX.parentSubNum,checkTrQty, checkTdQty,"notBeingUsed","notBeingUsed"); // make the table

qssBuildTableFills(objX.qNum, checkType, objSubType, 4, curNavLevObj, pullmenuIsneeded, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,selectIsNeeded); // make the buttons
...
qssBuildDropMenuBoxes(objParent.parentNode.qNum,9,objSubType,curNavLevObj,selectIsNeeded);
...
qssBuildTableFills(objX.qNum, objType, objSubType, 4, objX, 0, "notUsed", "notUsed", objIdArray[7], objParent.qNum, objX.parentSubNum,0); // make the borders for the object
...
//Body class
// macSa1.5+ bug adding className to body before something appends, causes it to create double inserts of the elements // not sure if this should go before the insertBefores for speed, but it could also mess up the  insertBefores if it is put before the insertBefores needs testing
qDocBody.className = qDocBodyClassName;
if(qBodyIsOn){
...
function qssBuildDropMenuBoxes(objQumber,objType,objSubType,curNavLevObj,selectIsNeeded){
...
(menuBox_Levl > scrollBoxesNeeded)? qssBuildDropMenuBox(qDropMnuDiv,"q"+(objQumber+1),menuBox_Levl): null; 
qssGetElmInVar(qDropMnuDiv,"q"+(objQumber+1)+"bodymen"+menuBox_Levl+"holderdiv","DIV").appendChild(d0);
...
qssBuildTableFills(objQumber+1,objType,objSubType,4, menu[c],true,"notUsed","notUsed",0,0,0,selectIsNeeded); // this fills in the menus for each box built
...
// qssBuildDropMenuBox
function qssBuildDropMenuBox(putObjHere,objQumber,menuBox_Levl){ // this makes a scrollbox
...
var d2 = qDoc.createElement('div');     	//  MB_holder
...
function qssBuildTableFills(objQumber,objType,objSubType, divqty, divObject, pointer_isneeded, notUsed0, notUsed1, borderImg, parentqNum, parentSubNum, selectIsNeeded){
...
//qWin.status=(divObject +" | " + divqty + " | " + divlevel);
...
var divX = createElement(isObjTypeDivBox && 'div' || "a");
var classFront = "q"+objQumber+"c";
divX.className = (useSelected)? classFront+"p"+ classFront : classFront;
...
div2 = qDoc.createElement("A");// change div two into a aElm
//:link    { color: red }
...
div2.onmouseover=div2.onmouseout=div2.onmousedown=div2.onmouseup=div2.onclick= qssMenuItem_event;
...
divX.className = divX.className + " qreSizer";
...
//alert(cc +"|||"+ objType +"|||"+ objQumber +"|||"+ qGBID("qBody").innerHTML);
//page 7
if(objType == 7){
...
//alert(qLayoutDiv.innerHTML);
} // this is the end of the divFill function 
...
var subMB_holder_Pos=qssGetPosElm(subMB_holder);// left and top positions
...
///var objNumber = parseInt(objID.substring(1)); // The Object's Number, the number that follows the q . i.e. 5 or 9 or etc.
var objChangeX = objX.parentNode.parentNode; // this id the first div or the td
...
if(objX.getElementsByTagName("b")[0]){
objX.alignClassName = "q"+ objX.qNum + "ctda"; // it used to pass along parent align classname to child to set css text and vertical align . It allows Parent box objects to children content boxes. 
//v1 qssBuildTable(objX.qNum, objType, objSubType, objParent.qNum , objX.parentSubNum, checkTrQty, checkTdQty,"notBeingUsed","notBeingUsed"); // make the table
}
qDocBodyClassName =  "q"+objX.qNum+"cz";
////////qDocBody.className =   "q"+objX.qNum+"cz"; //NOTE this will probally need tobe moved to onload
//alert(qDocBody.outerHTML);
...
*/
//00001 var imgCssPostion = ["top repeat-x","bottom repeat-x","left repeat-y","right repeat-y","left top no-repeat","right top no-repeat","left bottom no-repeat","right bottom no-repeat"];
...
//00001 var bgBorderStrType = (bgBorderType>0 && tdQTY>1)? (cc == 0)? (objType!=8)?'1010': '1100': (cc + 1 != tdQTY)? '0000': (objType!=8)?'0101':'0011':'1111'; // create a string for a tube
// It is posfible to put objType9 in emkbz where p is moved into b basically padding is moved into border 
// 2008/11/28 I moved b in front of 1111 and I'm add background into b from k
var divStr = "embz"; // divString
//var divStr = "embpz"; // divString
//00009 var divStr = "emkbpz"; // divString
//00001 var divStr = (objType!=9)? "emkb"+"1111"+bgBorderStrType + "pz" : "emkbpz"; // divString
//var divStr = (objType!=9)? "emk"+"1111"+bgBorderStrType + "bpz" : "emkbpz"; // divString
var trueDivQty = -1;
//00009 var trueBdrQty = -1;
//alert(divStr);
for(var ccc=0; ccc < divqty;ccc++){//forDivs
var zSizeClasses = "";
var classMid = divStr.charAt([ccc]);
trueDivQty++; // added this on build 49
eval("var div"+(trueDivQty)+"=qDoc.createElement('div');");
var divX = eval("div"+trueDivQty);
//0009 var classMid = divStrX;
...
// class front was wrong and added to many calles names on selected menu items repaired 000009, but I could be wrong si I'm keping
// a note here
divX.className = (useSelected)? classFront+"p"+classMid+ " q"+objQumber+classMid : classFront+classMid;
//00009 divX.className = (useSelected)? classFront+"p"+classMid+" "+ classFront+classMid : classFront+classMid;
...
if(classMid=="b"){
divX.className = divX.className + " qBrdBox";
}
else if(classMid=="z"){
...
if(isBgImgOn){//isBorder
 divX.style.background="url("+stackPath+"b/"+bgImageFolder+"/"+(trueBdrQty+1)+".gif) "+imgCssPostion[trueBdrQty]; 
}//isBorder
